...
always @(posedge CLK) begin
    if(~RST) begin
        ...
        if(states == 2'd3) begin   //Run
            ...
            //One or more Conditions(Parameters, FinishFlagIndicator) Corrupted
                ...
                if(FinishCount < 5'd24) FinishCount <= FinishCount + 5'd1; //Count for 20, and then Go Back to IDLE
                    ...
                    states <= 2'd0;
                    if (FinishFlagIndicator) FinishFlag <= 1'b1;
                end
            end
        end//...Run
        
    end
    else begin //RST == 1: Reset all register
		//Outputs
		Out_ValidWriteBack	<=	1'd0;
		Out_DataWriteBack	<=	`BIT_PSUM'd0;
		
		Out_ena_WeightSRAM 	<= 	`PE_COL'd0;
		Out_wea_WeightSRAM 	<=	`PE_COL'd0;
		Out_dina_WeightSRAM	<=	`BIT_DATA'd0;
		...
		Out_addrb_PsumSRAM	<=	{(`PE_COL*`BIT_DPSRAM){1'b0}};
		Out_ena_PsumSRAM	<=	`PE_COL'd0;
		Out_enb_PsumSRAM	<=	`PE_COL'd0;
		
		Out_wea_InputSRAM	<=	`PE_ROW'd0;
		Out_ena_InputSRAM	<=	`PE_ROW'd0;
		Out_dina_InputSRAM	<=	`BIT_DATA'd0;
		Out_addra_InputSRAM	<=	`BIT_SPSRAM'd0;
		...
		//Internals
		states			<=	2'd0;
		Instr_In		<=	`BIT_INSTR'd0;
		FinishFlag		<=	1'd0;
		Out_addra_WeightSRAM_0	<=	`BIT_SPSRAM'd0;
		FinishFlagIndicator	<=	1'd0;
