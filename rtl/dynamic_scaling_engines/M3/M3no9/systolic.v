...
module systolic ( CLK, i_Data_I_In, i_Data_W_In, i_EN_W_In, i_EN_ID_In, i_Psum_In, o_Psum_Out,
                  ...
                  i_ZeroFlag_In, i_ZeroFlag_W);
input CLK;
input [`PE_ROW*`BIT_DATA_C-1:0] i_Data_I_In;
input [`PE_COL*`BIT_DATA_C-1:0] i_Data_W_In;
...
input [`PE_COL-1:0] i_ZeroFlag_W;
//----------------------------------------------------------------

wire [`PE_COL*`PE_ROW*`BIT_DATA_C-1:0] Data_I_In;
wire [`PE_COL*`PE_ROW*`BIT_DATA_C-1:0] Data_I_Out;
wire [`PE_COL*`PE_ROW*`BIT_DATA_C-1:0] Data_W_In;
wire [`PE_COL*`PE_ROW*`BIT_DATA_C-1:0] Data_W_Out;
...
wire [`PE_COL*`PE_ROW-1:0] ZeroFlag_Weight_In;
wire [`PE_COL*`PE_ROW-1:0] ZeroFlag_Weight_Out;
//-----------------------------------------------------------------
...
generate
    for(i=0;i<`PE_COL;i=i+1) begin: loop_pe_col
        ...
        for (j=0;j<`PE_ROW;j=j+1) begin: loop_pe_row
            pe pe ( .CLK(CLK),
                    .Row_ID(j),
                    .Data_I_In(Data_I_In[`BIT_DATA_C*i+`BIT_DATA_C*`PE_COL*j+:`BIT_DATA_C]),
                    .Data_I_Out(Data_I_Out[`BIT_DATA_C*i+`BIT_DATA_C*`PE_COL*j+:`BIT_DATA_C]),
                    .Data_W_In(Data_W_In[`BIT_DATA_C*i+`BIT_DATA_C*`PE_COL*j+:`BIT_DATA_C]),
                    .Data_W_Out(Data_W_Out[`BIT_DATA_C*i+`BIT_DATA_C*`PE_COL*j+:`BIT_DATA_C]),
                    ...
                    .ZeroFlag_Weight_In(ZeroFlag_Weight_In[i*`PE_ROW+j]),
                    .ZeroFlag_Weight_Out(ZeroFlag_Weight_Out[i*`PE_ROW+j])
                    //-----------------------------------------------------------------------------------------------------------------------------------
                );                
                
                if(j>0) begin
                    assign Data_W_In[`BIT_DATA_C*i+`BIT_DATA_C*`PE_COL*j+:`BIT_DATA_C] = Data_W_Out[`BIT_DATA_C*i+`BIT_DATA_C*`PE_COL*(j-1)+:`BIT_DATA_C];
                    ...
                    //Zero-Skip----------------------------------------------------------------------------------------------------------------------------
                    assign ZeroFlag_Weight_In[i*`PE_ROW+j] = ZeroFlag_Weight_Out[i*`PE_ROW+(j-1)];
                    //-------------------------------------------------------------------------------------------------------------------------------------
                end
                else begin
                    assign Data_W_In[`BIT_DATA_C*i+`BIT_DATA_C*`PE_COL*j+:`BIT_DATA_C] = i_Data_W_In[`BIT_DATA_C*i+:`BIT_DATA_C];
                    ...
                    //Zero-Skip----------------------------------------------------------------------------------------------------------------------------
                    assign ZeroFlag_Weight_In[i*`PE_ROW+j] = i_ZeroFlag_W[i];
                    //-------------------------------------------------------------------------------------------------------------------------------------
                end
                
                if(i>0) begin
                    assign Data_I_In[`BIT_DATA_C*i+`BIT_DATA_C*`PE_COL*j+:`BIT_DATA_C] = Data_I_Out[`BIT_DATA_C*(i-1)+`BIT_DATA_C*`PE_COL*j+:`BIT_DATA_C];
                    //Zero-Skip----------------------------------------------------------------------------------------------------------------------------
                    assign ZeroFlag_In[i+`PE_COL*j] = ZeroFlag_Out[(i-1)+`PE_COL*j];
                    //-------------------------------------------------------------------------------------------------------------------------------------
                end
                else begin
                    assign Data_I_In[`BIT_DATA_C*i+`BIT_DATA_C*`PE_COL*j+:`BIT_DATA_C] = i_Data_I_In[`BIT_DATA_C*j+:`BIT_DATA_C];
                    //Zero-Skip----------------------------------------------------------------------------------------------------------------------------
