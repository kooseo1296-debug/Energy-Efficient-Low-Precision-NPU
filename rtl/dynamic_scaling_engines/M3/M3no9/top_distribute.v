...
// --- Wire Declarations Sorted by Number -------------------
    ...
    wire [(`BIT_DATA_C*`PE_ROW)-1:0] dina_InputSRAM;
    ...
    wire [`BIT_DATA_C-1:0] dina_InputSRAM_1;
    wire [`BIT_SPSRAM-1:0] addra_InputSRAM_1;
    generate
    for(i=0;i<`PE_ROW;i=i+1) begin
        assign dina_InputSRAM[i*`BIT_DATA_C+:`BIT_DATA_C] = dina_InputSRAM_1;
        assign addra_InputSRAM[i*`BIT_SPSRAM+:`BIT_SPSRAM] = addra_InputSRAM_1;
    end
    endgenerate
    
    //6
    wire [(`BIT_DATA_C*`PE_ROW)-1:0] InputSRAM_to_ZeroMask;
    
    //7
    wire [(`BIT_DATA_C*`PE_ROW)-1:0] ZeroMask_to_InputLoader;
    ...
    //10
    wire [(`BIT_DATA_C*`PE_COL)-1:0]      douta_WeightSRAM;
    ...
    wire [`PE_COL-1:0] ZeroFlag_W_Systolic;
//-------------------------------------------------------------------    
...
CGPE InputCGPE (
    .CLK(CLK),
    .RST(RST),
    .Input(dina_InputSRAM_0),
    .Output(dina_InputSRAM_1)
    );
...
CGPE WeightCGPE (
    .CLK(CLK),
    .RST(RST),
    .Input(dina_WeightSRAM_0),
    .Output(dina_WeightSRAM_1)
    );
...
systolic systolic ( 
    ...
    .i_ZeroFlag_W(ZeroFlag_W_Systolic)
    //----------------------------------------------
    );
...
generate for (i=0;i<`PE_ROW;i=i+1) begin: Loop_I
    blk_mem_gen_0_sp   InputSRAM (
        ...
        .clka(CLK),
        .dina(dina_InputSRAM[i*`BIT_DATA_C+:`BIT_DATA_C]),
        .douta(InputSRAM_to_ZeroMask[i*`BIT_DATA_C+:`BIT_DATA_C]),
        .ena(ena_InputSRAM[i]),
        .wea(wea_InputSRAM[i])
        );
    ZeroMask ZeroMask(
        .Input(InputSRAM_to_ZeroMask[i*`BIT_DATA_C+:`BIT_DATA_C]),
        .Disable(Disable_ZeroMask[i]),
        .Output(ZeroMask_to_InputLoader[i*`BIT_DATA_C+:`BIT_DATA_C])
        );
    //Zero-Skip ---------------------------------------------------
    ZeroDetect ZeroDetect_I(
        .Input(InputLoader_to_SysArr[i*`BIT_DATA_C +: `MAN]),
...
generate for (i=0;i<`PE_COL;i=i+1) begin: Loop_W
    blk_mem_gen_0_sp   WeightSRAM (
        ...
        .clka(CLK),
        .dina(dina_WeightSRAM[i*`BIT_DATA_C+:`BIT_DATA_C]),
        .douta(douta_WeightSRAM[i*`BIT_DATA_C+:`BIT_DATA_C]),
        .ena(ena_WeightSRAM[i]),
        .wea(wea_WeightSRAM[i])
        );
    //Zero-Skip ---------------------------------------------------
    ZeroDetect ZeroDetect_W(
        .Input(douta_WeightSRAM[i*`BIT_DATA_C+:`MAN]),
        .ZeroFlag(ZeroFlag_W_Systolic[i])
        );
    //-------------------------------------------------------------
