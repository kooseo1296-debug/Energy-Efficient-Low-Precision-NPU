...
module CGPE(
    ...
    output reg [`BIT_DATA-1:0] Output
    );
    integer i;
    ...
    assign Exponent_0 = (Absolute[`BIT_DATA-1]) ? `EXP'd4: 
                      (Absolute[`BIT_DATA-2]) ? `EXP'd3: 
                      (Absolute[`BIT_DATA-3]) ? `EXP'd2: 
                      (Absolute[`BIT_DATA-4]) ? `EXP'd1: `EXP'd0;
    ...
    assign Mantissa_0 = (Exponent_0 == `EXP'd0) ? {1'b0,Absolute[`MAN-1:0]} :
                      ...
                      (Exponent_0 == `EXP'd3) ? {1'b0,Absolute[`MAN+2:3]} : {1'b0, Absolute[`MAN+3:4]};
    
    assign RoundUp = (Exponent_0 == `EXP'd1) ? Absolute[0] :
                     (Exponent_0 == `EXP'd2) ? Absolute[1] : 
                     (Exponent_0 == `EXP'd3) ? Absolute[2] : 1'b0;
