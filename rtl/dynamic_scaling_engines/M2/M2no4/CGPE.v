...
module CGPE(
    ...
    assign Exponent_0 = (Absolute[`BIT_DATA-1]) ? `EXP'd6: 
                      (Absolute[`BIT_DATA-2]) ? `EXP'd5: 
                      (Absolute[`BIT_DATA-3]) ? `EXP'd4: 
                      (Absolute[`BIT_DATA-4]) ? `EXP'd3:
                      (Absolute[`BIT_DATA-5]) ? `EXP'd2: 
                      (Absolute[`BIT_DATA-6]) ? `EXP'd1: `EXP'd0;
    ...
    assign Mantissa_0 = (Exponent_0 == `EXP'd0) ? {1'b0,Absolute[`MAN-1:0]} :
                      ...
                      (Exponent_0 == `EXP'd5) ? {1'b0, Absolute[`MAN+4:5]} : {1'b0, Absolute[`MAN+5:6]};
    ...
    assign RoundUp = (Exponent_0 == `EXP'd1) ? Absolute[0] :
                     ...
                     (Exponent_0 == `EXP'd5) ? Absolute[4] : 1'b0;
