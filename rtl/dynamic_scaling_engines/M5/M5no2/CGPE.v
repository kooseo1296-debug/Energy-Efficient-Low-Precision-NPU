...
module CGPE(
    ...
    assign Exponent_0 = (Absolute[`BIT_DATA-1]) ? `EXP'd3: 
                      (Absolute[`BIT_DATA-2]) ? `EXP'd2: 
                      (Absolute[`BIT_DATA-3]) ? `EXP'd1: `EXP'd0;
    
    assign Mantissa_0 = (Exponent_0 == `EXP'd0) ? {1'b0,Absolute[`MAN-1:0]} :
                      (Exponent_0 == `EXP'd1) ? {1'b0,Absolute[`MAN:1]} :
                      (Exponent_0 == `EXP'd2) ? {1'b0,Absolute[`MAN+1:2]} : {1'b0,Absolute[`MAN+2:3]};
    
    assign RoundUp = (Exponent_0 == `EXP'd1) ? Absolute[0] :
                     (Exponent_0 == `EXP'd2) ? Absolute[1] : 1'b0;
